...
class AlfredSpeech {
  constructor() {
    ...
    this._offlineSupported = Boolean(
      window.WebSocket &&
  ...
  cancelSpeech() {
    ...
    try { this._ttsAudio.pause(); } catch (_) {}
    this._cleanupTtsAudio();
  }
  ...
  async _startOffline() {
    ...
    this._ws.onopen = () => {
      ...
      if (this.onStateChange) this.onStateChange("idle", { reason: "connected" });
    };
    ...
    this._ws.onmessage = (evt) => {
      ...
      if (msg.type === "idle") {
        ...
        if (this.onStateChange) this.onStateChange("idle", { reason: msg.reason || "ready" });
        return;
      }
    ...
    this._ws.onclose = () => {
      ...
      if (!this._active || this._useFallback) return;
  ...
  _startFallback() {
    if (!this._fallback.supported) {
      ...
      if (this.onStateChange) this.onStateChange("denied");
      return;
    }
    ...
    this._fallback.onWakeWord = () => { if (this.onWakeWord) this.onWakeWord(); };
    this._fallback.onCommand = (text) => { if (this.onCommand) this.onCommand(text); };
    this._fallback.onStateChange = (state) => {
      if (this.onStateChange) this.onStateChange(state, { reason: "fallback" });
    };
...
class BrowserSpeechFallback {
  constructor() {
    ...
    if (this.supported) this._initRecognition(SR);
  }
      ...
      if (!this._commandMode) {
        if (transcript.includes("alfred")) {
          ...
          if (afterWake.length > 2) {
            ...
            if (this.onCommand) this.onCommand(afterWake);
          }
        }
      } else if (isFinal) {
