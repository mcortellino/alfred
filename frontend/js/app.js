...
const alfred = {
    ...
    _applyLanguageUI() {
        ...
        if (settingsLangSel)
            settingsLangSel.value = this.lang;
    ...
    _diag(message) {
        ...
        if (log)
            log.textContent = this._diagLog.join("\n");
    },
    async _refreshVoiceStatus() {
        const summary = document.getElementById("diag-summary");
        if (summary)
            summary.textContent = "Checking backend voice status...";
        try {
            ...
            if (status.vosk_model_path)
                this._diag(`voice.vosk_path ${status.vosk_model_path}`);
            if (status.reason && status.reason !== "ok")
                this._diag(`voice.reason ${status.reason}`);
        }
        catch (err) {
            if (summary)
                summary.textContent = "Voice backend: UNREACHABLE";
    ...
    async _refreshTtsStatus() {
        ...
        if (ttsSummary)
            ttsSummary.textContent = "TTS: checking...";
        try {
            ...
            if (!statusRes.ok)
                throw new Error(`TTS status HTTP ${statusRes.status}`);
            ...
            if (prefSel && status.voice_preference)
                prefSel.value = status.voice_preference;
            ...
            if (status.selected_voice?.id)
                this._diag(`tts.selected ${status.selected_voice.id}`);
        }
        catch (err) {
            if (ttsSummary)
                ttsSummary.textContent = "TTS: UNREACHABLE";
    ...
    // ── Button wiring ────────────────────────────────────────────────
    _wireButtons() {
        document.getElementById("btn-radio").addEventListener("click", () => this._restoreHomeSkillView("radio") || this._reopenActiveRadio() || this.processCommand("play radio"));
        document.getElementById("btn-tv").addEventListener("click", () => this._restoreHomeSkillView("tv") || this._reopenActiveTv() || this.processCommand("show tv"));
        document.getElementById("btn-shopping").addEventListener("click", () => this.processCommand("show shopping list"));
        document.getElementById("btn-calendar").addEventListener("click", () => this.processCommand("show calendar"));
        document.getElementById("btn-timer").addEventListener("click", () => this._restoreHomeSkillView("timer") || this._showTimerSetup());
        const homeBtn = document.getElementById("btn-home");
        if (homeBtn)
            homeBtn.addEventListener("click", () => this._goHomeView());
        const cmdBtn = document.getElementById("btn-command");
        if (cmdBtn)
            cmdBtn.addEventListener("click", () => this._toggleCommandRow());
        const stopBtn = document.getElementById("btn-stop");
        if (stopBtn)
            stopBtn.addEventListener("click", () => this._stopEverything(true));
    },
    _wireTextInput() {
    ...
    _isStopCommand(command) {
        ...
        if (stopTerms.includes(stripped))
            return true;
    ...
    _handleResult(result) {
        ...
        switch (action) {
            ...
            case "tv_off":
                ...
                break;
            case "tv_list":
                this._showChannelPicker(data);
                break;
            case "shopping_list":
            case "shopping_updated":
                this._showShopping(data);
                break;
            case "calendar_open":
          ...
          <div class="tv-player" style="position:relative">
            ...
            if (typeof Hls !== "undefined" && Hls.isSupported()) {
                ...
                hls.on(Hls.Events.MANIFEST_PARSED, () => {
                    try {
                        const sel = document.getElementById('tv-res-select');
                        if (!sel)
                            return;
                        // Clear and add Auto
                        sel.innerHTML = '';
                ...
                hls.on(Hls.Events.ERROR, (_e, data) => {
                    if (data.fatal) {
                        video.insertAdjacentHTML("afterend", `<p style="color:var(--c-error);padding:8px;font-size:.85rem">
        ...
        <div class="tv-shell tv-shell-full">
          <div class="tv-player" style="position:relative">
            ...
            if (typeof dashjs !== "undefined" && dashjs.MediaPlayer) {
                ...
                // Populate resolution selector for DASH
                try {
                    const sel = document.getElementById('tv-res-select');
                    if (sel) {
                        sel.innerHTML = '';
                        ...
                        infos.forEach((info, idx) => {
                            const label = info.height ? `${info.height}p` : `${Math.round((info.bitrate||0)/1000)}kbps`;
                            const opt = document.createElement('option');
                            opt.value = String(idx);
                            opt.text = label;
                            sel.appendChild(opt);
    ...
    _stopTvPlayback(clearUI = false) {
        ...
        if (video && video._dash && typeof video._dash.reset === "function") {
            video._dash.reset();
        }
        if (video) {
            video.pause();
      ...
      <div id="calendar-event-dialog" role="dialog" aria-modal="true" aria-labelledby="calendar-event-title">
        ...
        if (titleInput) {
            ...
            titleInput.addEventListener("keydown", (e) => {
                if (e.key === "Enter")
                    this._submitCalendarEventDialog(dateKey);
            });
        }
    ...
    _submitCalendarEventDialog(dateKey) {
        ...
        const hourInput = document.getElementById("calendar-event-dialog-hour");
        const minuteInput = document.getElementById("calendar-event-dialog-minute");
        if (!titleInput || !hourInput || !minuteInput)
            return;
        const title = titleInput.value.trim();
    ...
    _renderCalendarView(view, referenceDate, events, transitionDirection = "") {
        ...
        if (view === "week") {
            ...
            const festivityDates = new Set(Object.keys(eventsByDate).filter(dateKey => (eventsByDate[dateKey] || []).some(ev => this._isFestivityEvent(ev))));
            const dayCards = [];
            for (let i = 0; i < 7; i++) {
    ...
    _startCustomTimer() {
        ...
        if (it) {
            if (m)
                parts.push(`${m} minut${m !== 1 ? "i" : "o"}`);
            if (s)
                parts.push(`${s} second${s !== 1 ? "i" : "o"}`);
    ...
    _cancelTimer(respond = true) {
        ...
        if (timerPanel)
            this._hideContent();
        if (respond)
            this.respond(this._isItalian() ? "Timer annullato, signore." : "Timer cancelled, sir.");
    },
    _timerDone(label) {
        const it = this._isItalian();
        this._hideContent();
    ...
    _stopTimerAlarm() {
        if (!this._timerAlarm)
            return;
        if (this._timerAlarm.intervalId)
            clearInterval(this._timerAlarm.intervalId);
        if (this._timerAlarm.timeoutId)
            clearTimeout(this._timerAlarm.timeoutId);
    ...
    _restoreHomeSkillView(view) {
        ...
        if (!ca || !canvas)
            return false;
        if (!ca.classList.contains("hidden"))
            return false;
        if (this._homeHiddenView !== view)
            return false;
        if (!ca.innerHTML.trim())
            return false;
        ca.classList.remove("hidden");
        if (idle)
            idle.classList.add("hidden");
...
function bootstrapAlfred() {
    if (alfred.face)
        return true;
    if (!document.getElementById("alfred-face-container"))
        return false;
    if (typeof AlfredFace === "undefined" || typeof AlfredSpeech === "undefined")
        return false;
    alfred.init();
    return true;
}
